...
CREATE DEFINER=`{ADMIN}`@`{SERVER}` PROCEDURE `CorsAddUpdate`(IN `a` CHAR(255), IN `u` LONGTEXT, IN `i` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER=`{ADMIN}`@`{SERVER}` PROCEDURE `CorsUpdate`(IN `i` BIGINT, IN `a` CHAR(255), IN `u` LONGTEXT, IN `ip` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
 COMMENT 'Add/Update cors'
BEGIN
